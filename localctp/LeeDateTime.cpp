...
CLeeDateTime::CLeeDateTime(const struct tm& _tm)  
{
	...
	{
		// timegm/_mkgmtime interprets the tm as UTC; subtract the pinned offset
		// so the fields are treated as Shanghai wall clock instead of host-local.
...
#ifdef _WIN32
	// GetSystemTime gives UTC; SystemTimeToFileTime inside the SYSTEMTIME
	// ctor packs face-value, so the resulting m_dt is a UTC-face-value
	// serial. Add the pinned offset to shift to Shanghai-face-value.
	::SYSTEMTIME sysTime1 = { 0 };
	::GetSystemTime(&sysTime1);
	return CLeeDateTime(sysTime1) + CLeeDateTimeSpan(0, CLeeDateTime::GetTimeZone(), 0, 0);
#endif
#if defined(__linux__) || defined(__APPLE__)
	// gettimeofday returns UTC seconds; time_t_to_double then adds the
	// pinned offset, so m_dt ends up as Shanghai-face-value regardless
	// of the host's TZ.
	::timeval tv;
...
int CLeeDateTime::GetTimeZone() {
	// Pinned to UTC+8 (Asia/Shanghai). CTP trading hours are defined in CST,
	// and the trading-day rollover heuristic in StaticGetTradingDay (now+4h)
	// is calibrated for the CN futures night session. Returning the host's
	// offset would make trading-day boundaries depend on the dev machine's
	// TZ, which is the bug we're fixing.
	return 8;
}
